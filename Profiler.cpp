...
void Profiler::stop()
{
    ...
    // Log results
    Logger::info("Profiler results: %d calls (%d uniques) registered in %.3f sec", total_calls, sorted_funcs.size(), (float)(timeGetTime() - profiling_start) / 1000.0f);
    Logger::info("%-10s %-10s %-10s %-10s %s", "ncalls", "tottime", "percall", "percentage", "funcname");
    for (auto it : sorted_funcs)
        Logger::info("%-10d %-10.3f %-10.3f %-10.3f %s", it->getNCalls(), (float)it->getTotalTime() / 1000.0f, (float)it->getTimePerCall() / 1000.0f, it->getPercentage(), it->getName().c_str());
}
