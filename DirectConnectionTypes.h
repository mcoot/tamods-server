				...
				for (json::iterator prop_it = curItem.begin(); prop_it != curItem.end(); ++prop_it) {
					...
					case GameBalance::ValueType::BOOLEAN:
						if (!prop_it->is_boolean()) {
							Logger::error("Unable to read item prop config for propId %d on item %d: wrong type (should be boolean)", propId, itemId);
							continue;
						}
					...
					case GameBalance::ValueType::INTEGER:
						if (!prop_it->is_number_integer()) {
							Logger::error("Unable to read item prop config for propId %d on item %d: wrong type (should be integer)", propId, itemId);
							continue;
						}
					...
					case GameBalance::ValueType::FLOAT:
						if (!prop_it->is_number()) {
							Logger::error("Unable to read item prop config for propId %d on item %d: wrong type (should be float)", propId, itemId);
							continue;
						}
					...
					case GameBalance::ValueType::STRING:
						if (!prop_it->is_string()) {
							Logger::error("Unable to read item prop config for propId %d on item %d: wrong type (should be string)", propId, itemId);
							continue;
						}
