...
namespace DCServer {
            ...
            for (json::iterator elem_it = j.begin(); elem_it != j.end(); ++elem_it) {
                ...
                for (json::iterator prop_it = curElem.begin(); prop_it != curElem.end(); ++prop_it) {
                    ...
                    if (!readPropValFromJson(*prop_it, propDefIt->second.getType(), propVal)) {
                        Logger::error("Unable to read item prop config for propId %d on elem %d: wrong type (should be %s)",
                            propId, elemId,
    ...
    public:
        ...
        bool fromJson(const json& j) {
            ...
            IngameMsgDetails details;
            details.doShow = ingameMessageJson["do_show"].get<bool>();
            if (details.doShow) {
                ...
                details.message = ingameMessageJson["message"].get<std::string>();
                details.time = ingameMessageJson["time"].get<float>();
                details.priority = ingameMessageJson["priority"].get<int>();
            }
