...
namespace TCP {
	...
	public:
		...
		void stop() {
			Logger::debug("stopping client... error code is %d: %s", error_state.value(), error_state.message());
			stopped = true;
			socket.close();
			deadline.cancel();
		}
	...
	public:
		...
		void start(std::function<void()> connect_handler = NULL, std::function<void(boost::system::error_code&)> stop_handler = NULL) {
			stopped = false;
